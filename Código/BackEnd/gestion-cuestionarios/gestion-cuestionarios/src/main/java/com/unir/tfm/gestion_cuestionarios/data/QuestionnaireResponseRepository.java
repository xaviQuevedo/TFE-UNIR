...
public interface QuestionnaireResponseRepository extends JpaRepository<QuestionnaireResponse, Long> {
    ...
    @Query(value = """
            SELECT
                ...
                TO_CHAR(qr.created_at, 'YYYY-MM-DD') AS responseDate
            FROM
            ...
            WHERE
                ...
                AND qr.created_at = (
                    SELECT MAX(sub_qr.created_at)
                    FROM questionnaire_responses sub_qr
                    WHERE sub_qr.patient_id = qr.patient_id
                    AND sub_qr.question_id = qr.question_id
                    AND sub_qr.questionnaire_id = qr.questionnaire_id
                )
            """, nativeQuery = true)
