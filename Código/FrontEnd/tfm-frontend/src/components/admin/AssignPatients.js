...
const AssignPatients = () => {
  const {
    physiotherapists,  // Cambiado a plural
    patients,
  ...
  return (
    <div className="assign-patients-container">
      ...
      <div className="form-group">
        ...
        >
          ...
          {physiotherapists.map((physiotherapist) => (
            <option key={physiotherapist.user_id} value={physiotherapist.user_id}>
              {physiotherapist.name} {physiotherapist.lastName}
            </option>
          ))}
        </select>
      </div>
      ...
      <button
        ...
        onClick={() => handleAssign(navigate)}  // Simplificado el uso de navigate
      >
        Asignar Pacientes
      </button>
    </div>
